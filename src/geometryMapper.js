...
class GeometryMapper {
   ...
   */
	async mapElementsToSpaces() {
		...
		const elementsData = await Promise.all(elements.map(({ elementId }) => ({
			elementId,
			bbox: this.headlessViewer.getElementBoundingBox(elementId)
		})))

		const spacesData = await Promise.all(spaces.map(({ elementId }) => ({
			elementId,
			bbox: this.headlessViewer.getElementBoundingBox(elementId),
		})))

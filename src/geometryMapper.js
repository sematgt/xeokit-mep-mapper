...
const ggfmServiceWithFactory = require(__base + 'src/backend-src/ggfmServiceWithFactory')


class GeometryMapper extends ggfmServiceWithFactory {
  /**
   * 
   * @param {*} architectureModelId exon model id
   * @param {*} engineeringModelId
   */
  constructor(architectureModelId, engineeringModelId) {
    this.architectureModelId = architectureModelId
    this.engineeringModelId = engineeringModelId
    this.viewer = new HeadlessViewer()
  }
  ...
  async mapElementsToSpaces() {
    ...
    const bimServiceClient = await this.factory.newObject('bimServiceClient')
